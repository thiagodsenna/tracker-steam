...
async function buscarJogosSimilares(steamId) {
    ...
    try {
        ...
        container.innerHTML = data.items.map(jogo => `
            <a href="#" onclick="fecharModal(); abrirModal(${jogo.id})" title="Ver na Steam: ${jogo.id} - ${jogo.name}" class="group bg-neutral-950 border border-neutral-800 rounded-lg overflow-hidden hover:border-emerald-500/50 transition-all flex flex-col justify-between shadow-md">
