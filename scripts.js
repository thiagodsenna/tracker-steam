...
async function buscarOutrasReleases(tituloOriginal, steamId, currentFeedlyId) {
    ...
    // Força o container a ser uma única coluna vertical ocupando 100% do espaço
            ...
            return `
                <button type="button" 
                    ...
                    <div class="flex items-center gap-2 w-full">
                        ...
                        ${novoJogo.release.versao ? `<span class="bg-neutral-900 text-[9px] text-neutral-300 px-1.5 py-0.5 rounded font-mono font-bold shrink-0 ml-0">${novoJogo.release.versao}</span>` : ''}
                    </div>
                </button>
            `;
        }).join('');
...
async function buscarReviewsSteam(steamId) {
    try {
        ...
        if (json.success && json.reviews && json.reviews.length > 0) {
            ...
            document.getElementById('total-reviews-section').innerHTML = ` | ${totalReviews}`;
                    ...
                    <div onclick="const container = this.querySelector('.review-container'); const dots = this.querySelector('.review-dots'); const btnEl = this.querySelector('.review-toggle-btn'); if (container && btnEl) { container.classList.toggle('max-h-[5.5rem]'); if (dots) dots.classList.toggle('hidden'); btnEl.textContent = container.classList.contains('max-h-[5.5rem]') ? '' : 'MENOS'; }" 
                        ...
                        <div class="flex items-center justify-between gap-2 mb-2 flex-wrap">
                            ...
                            <div class="flex items-center gap-1.5 text-neutral-500 text-[10px] ml-auto shrink-0">
                                ...
                                <span class="flex items-center gap-1">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-neutral-500 inline shrink-0"><rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                                    ${dataFormatada}
                                </span>
                            </div>
                        </div>
            ...
            return `
                <div class="space-y-1.5">
                    <div class="flex justify-between items-center text-[11px] font-bold">
                        ...
                        <span class="${b.textCor} font-mono text-[11px] sm:text-xs bg-neutral-900 px-1.5 py-0.5 rounded border border-neutral-800">${formatarHoras(b.tempo)}</span>
                    </div>
